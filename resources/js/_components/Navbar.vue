<template>
    <nav class="flex items-center justify-between flex-wrap p-2">
        ...
        <div class="w-4/6 flex items-center flex-shrink-0">
            <div class=" text-sm flex-grow" v-if="isLoggedIn === true">
                <div class="w-full mx-auto">
                    <input id="docsearch" class="transition border border-transparent placeholder-gray-900 rounded-lg bg-gray-300 py-2 pr-4 pl-4 block w-full appearance-none leading-normal ds-input" type="text" placeholder="Search the docs (Press &quot;/&quot; to focus)" autocomplete="off" spellcheck="false" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-label="search input" aria-owns="algolia-autocomplete-listbox-0" dir="auto" />
                </div>
            </div>
        </div>
        <div class="w-1/6 flex justify-end flex-shrink-0">
            <router-link class="flex" to="/auth/signin" v-if="isLoggedIn !== true">
...
export default {
    data: function () {
        return { logo: logo, login: login, siteName: _siteName, isLoggedIn: this.$store.getters.isLoggedIn }
    }
}
</script>
